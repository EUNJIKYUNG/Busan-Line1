...
void CTableManage::LoadDatabase()
{
	...
	MakeRelationShip<DefaultComparator>(&VECTOR_CLASS(StopPtnHeader), 0, &TEMP_VECTOR_CLASS(StopPtnRoutes));		//부모 자식 관계
	MakeRelationShip<DefaultComparator>(&TEMP_VECTOR_CLASS(StopPtnRoutes), 0, &TEMP_VECTOR_CLASS(EventLists));		//부모 자식 관계
	MakeRelationShip<DefaultComparator>(&VECTOR_CLASS(VideoContents), 0, &TEMP_VECTOR_CLASS(VideoIndexList));		//부모 자식 관계
	MakeRelationShip<DefaultComparator>(&VECTOR_CLASS(AudioContents), 0, &TEMP_VECTOR_CLASS(AudioIndexList));		//부모 자식 관계
	MakeRelationShip<DefaultComparator>(&VECTOR_CLASS(PIDContents), 0, &TEMP_VECTOR_CLASS(PIDIndexList));		//부모 자식 관계
	MakeRelationShip<DefaultComparator>(&VECTOR_CLASS(UpdateFileTypeList), 0, &TEMP_VECTOR_CLASS(UpdateFile));		//부모 자식 관계
	MakeRelationShip<DefaultComparator>(&VECTOR_CLASS(UpdateFileTypeList), 1, &TEMP_VECTOR_CLASS(UpdateTarget));		//부모 자식 관계
	MakeRelationShip<DefaultComparator>(&VECTOR_CLASS(EDDIndex), 0, &TEMP_VECTOR_CLASS(EDDIndexList));		//부모 자식 관계
