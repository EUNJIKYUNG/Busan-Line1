/*
	완성형 <-> 유니코드 양방향 코드변환 프로그램
	- 입력: 유니코드 또는 KS완성형
	- 출력: KS완성형 또는 유니코드(BE, LE, UTF-8)

	2009년 8월 23일
	(C) 강승식, nlpkang@gmail.com
*/
#include <stdio.h>
#include <string.h>
#include <ctype.h>
...
enum { UTF8='t', UTF8_BOM='T', UNICODE_BE='U', UNICODE_LE='u', KSC5601='k' };
...
void conv_uni2ksc(uint16_t *uni, uint8_t *ksc)
{
	uint8_t *q=ksc;
	int i, ksccode;
	int nUni2KSC=sizeof(uni2ksc) / sizeof(uni2ksc[0]);
	for (i=0; uni[i]; i++) {
		ksccode = bsearch_unitab(uni[i], uni2ksc, nUni2KSC);
		if (ksccode > 0) {
			*q++ = (ksccode >> 8) & 0xFF;
			*q++ = ksccode & 0xFF;
...
char set_inputmode(FILE *fpin, char in_mode) {
	...
	c = fgetc(fpin);
	if (c == 0xFE && (d=getc(fpin)) == 0xFF)
		return UNICODE_BE;
	if (c == 0xFF && (d=getc(fpin)) == 0xFE)
		return UNICODE_LE;
	if (c == 0xEF && (d=getc(fpin)) == 0xBB && (d=getc(fpin)) == 0xBF)
		return UTF8;
	fseek(fpin, 0, 0);
	return in_mode;
}

// Unicode 또는 UTF 문자 입력
// 유니코드 -> 완성형 프로그램에 반대방향 기능을 추가했더니 함수명이 안 맞음! -.-
...
int get_nextcode(FILE *fpin, int in_mode)
{
		...
		if (unicode & 0x80)
			...
			if (c == '+') {
				...
				if (isxdigit(c)) {
					...
					if (i != 6) {
						ungetc(c, fpin);
						for (j=i-1; j >= 0; j--) ungetc(a[j], fpin);
						ungetc('+', fpin);
						return unicode;
					}
					unicode = strtol(a, NULL, 16);
				} else {
					ungetc(c, fpin); ungetc('+', fpin);	// 역순으로...
				}
			} else ungetc(c, fpin);
		}
		return unicode;	// 유니코드 -> 완성형 프로그램에 반대방향 기능을 추가했더니 변수명이 안 맞음! -.-
	case UNICODE_BE:
		unicode = (getc(fpin) << 8) | getc(fpin);
		if ((unicode & 0xFC00) == 0xD800) {	// SMP 영역의 4바이트 인코딩된 문자 -> 21비트로
			unicode = (((unicode >> 6) & 0xF) + 1) << 16 | (unicode & 0x3F) << 10;
			unicode |= ((getc(fpin) << 8) | getc(fpin)) & 0x3FF;
		}
		return unicode;
	case UNICODE_LE:
		unicode = getc(fpin) | (getc(fpin) << 8);
		if ((unicode & 0xFC00) == 0xD800) {	// SMP 영역의 4바이트 인코딩된 문자 -> 21비트로
	...
	default:
		...
		if ((c & 0xF0) == 0xE0) {	// 3바이트 인코딩
			d = getc(fpin); e = getc(fpin);
			if ((d & 0xC0) == 0x80 && (e & 0xC0) == 0x80)
				return ((c << 12) & 0xF000) | ((d << 6) & 0x0FC0) | e & 0x3F;
			else {	// UTF-8 ERROR
				ungetc(e, fpin);
				return (c << 8 | d);
			}
		...
		if ((c & 0xE0) == 0xC0) {	// 2바이트 인코딩
			d = getc(fpin);
			if ((d & 0xC0) == 0x80)
				return ((c << 6) & 0x07C0) | (d & 0x3F);
			else return (c << 8 | d);	// UTF-8 ERROR
		}
		if ((c & 0xF8) == 0xF0) {	// 4바이트 인코딩
			d = getc(fpin); e = getc(fpin); f = getc(fpin);
			if ((d & 0xC0) == 0x80 && (e & 0xC0) == 0x80 && (f & 0xC0) == 0x80)
				return ((c << 18) & 0x1C0000) | ((d << 12) & 0x3F000) | ((e << 6) & 0x0FC0) | f & 0x3F;
			else {	// UTF-8 ERROR
				ungetc(f, fpin); ungetc(e, fpin);	// 역순으로...
				return (c << 8 | d);
			}
		}
		return c;
...
void put_BOM(FILE *fpout, int out_mode)
{
	...
	case UTF8_BOM:	// UTF-8 BOM
		...
		break;
	case UNICODE_BE:	// Unicode Big Endian BOM
		putc(0xFE, fpout); putc(0xFF, fpout);
		break;
	case UNICODE_LE:	// Unicode Little Endian BOM
...
{
	if (unicode == 0x00FF62 || unicode == 0x00FF63) {
		fprintf(fpout, "%c", '\'');	// 작은 따옴표
		return 1;
	}

	if (unicode == 0x0F08DC || unicode == 0x0F09DC) {
		fprintf(fpout, "%c", '\'');	// 작은 따옴표
		return 1;
	}

	if (unicode == 0x0F0ADC || unicode == 0x0F0BDC) {
		fprintf(fpout, "%c", '\"');	// 큰 따옴표
		return 1;
	}

/*	if (unicode == 0x0025C9) {	// 변환표에서 처리했음.
		fprintf(fpout, "%c", '\"');	// 큰 따옴표
		return 1;
	}
*/
	if (unicode == 0x00F85E) {
		fprintf(fpout, "%c", ' ');	// 가운데점? 공백으로 처리...
		return 1;
	}
